...
export class ApiSurveyService {
  private url = 'https://itdg-ng-questionnaire-backend.onrender.com/api/';
  ...
  postSurvey(newSurvey: Survey) {
    this.http
      .post(this.url + 'post-survey', newSurvey, httpOptions)
      .subscribe((res) => {
        console.log(res);
      });
  }
  ...
  putSurvey(editedSurvey: Survey) {
    return this.http
      .put<{ msg: string; survey: Survey }>(
        this.url + 'edit-survey/' + editedSurvey.surveyId,
        editedSurvey
      )
      .subscribe((res) => {
  ...
  deleteSurvey(survey: Survey): Observable<Survey> {
    return this.http.delete<Survey>(
      this.url + 'delete-survey/' + survey.surveyId
    );
  }

  postAnswer(answer: Answer) {
    this.http
      .post(this.url + 'post-answer', answer, httpOptions)
      .subscribe((res) => {
        console.log(res);
      });
  }
